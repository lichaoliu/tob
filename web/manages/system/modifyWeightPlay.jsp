	    ...
	    function save() {
	       ...
	       // alert(plays + "---" + polls);
	        ...
	        if (confirm("确定要保存此玩法配置？")) {
	            ...
	            document.form1.action = "/manages/postManagesServlet?action=modify_weight_play&playId=" + playId + "&playCodes=" + plays + "&pollCodes=" + polls + "&weightId=" + weightId + "&lotteryCode=" + lotteryCode;
	            form1.submit();
	        }
	    }
...
<body>
<input type="hidden" id="playId" name="playId" value="${playId}">
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table">
    <tr>
        <td class="title" style="text-align:left;">系统参数设置 >> 出票口管理>>修改玩法</td>
    </tr>
    <tr>
                ...
                <table  width="100%" border="1" cellpadding="4" cellspacing="0" class="content">
	                <c:if test="${lotteryPlayList != null && fn:length(lotteryPlayList) != 0}">
	            		...
	            		<c:forEach items="${lotteryPlayList}" var="lotteryPlay" varStatus="cont">
	            			<c:choose>
	            				<c:when test="${fn:contains(weightPlay.playCodes, lotteryPlay.code)}">
	            					<tr>
										<td>
											<input name="myCheckbox" type="checkbox" id="check${lotteryPlay.code}" value="${lotteryPlay.code}" checked="checked">
										</td>
				                        <td align="center">
											${lotteryPlay.name}						
										</td>
										<td>
				            				<c:forEach var="pollBean" items="${lotteryPlay.lotteryPollList}">
								            	...
								            	<c:if test="${fn:contains(weightPlay.pollCodes, pollBean.code)}">
				            						<input name="pollCheckbox${lotteryPlay.code}" type="checkbox" id="check${pollBean.code}" value="${pollBean.code}" checked="checked">${pollBean.name}
								            	</c:if>
								            </c:forEach>
										</td>
			                   		</tr>
	            				...
	            				<c:otherwise>
	            					<c:if test="${!fn:contains(plays, lotteryPlay.code)}">
				            			<tr>
											<td>
												<input name="myCheckbox" type="checkbox" id="check${lotteryPlay.code}" value="${lotteryPlay.code}">
											</td>
					                        <td align="center">
												${lotteryPlay.name}						
											</td>
