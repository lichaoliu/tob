...
<head>
    ...
    <script type="text/javascript">
        ...
        function toEditOpenBasketball(id){
            ...
            var page= $("#pageId").val();
            window.location.href='${pageContext.request.contextPath}/manages/openAward?action=editOpenBasketball&id='+id+'&matchNo='+encodeURIComponent(encodeURIComponent(matchNo))+"&mainTeam="+encodeURIComponent(encodeURIComponent(mainTeam))+"&guestTeam="+encodeURIComponent(encodeURIComponent(guestTeam))+"&matchName="+encodeURIComponent(encodeURIComponent(matchName))+"&operatorsAward="+encodeURIComponent(encodeURIComponent(operatorsAward))+"&page="+page;
        }
    </script>
</head>
<body>
    ...
    <tr>
        <td style="padding: 4px;">
            ...
            <form name="form1" method="post"
                  action="${pageContext.request.contextPath}/manages/openAward?action=openBasketball">
                ...
                <c:if test="${pageBean.pageContent!=null}">
                    <c:forEach var="basketball" items="${pageBean.pageContent}" varStatus="cont">
                        <tr>
                            ...
                            <td align="center">让分${basketball.letBall}<br/>大小分${basketball.preCast}</td>
                            <td align="left">
                                <fmt:formatDate value="${basketball.endFuShiTime}" pattern="yyyy-MM-dd"/><br/>
                                <fmt:formatDate value="${basketball.endFuShiTime}" pattern="HH:mm:ss"/>
                            </td>
                            <td align="left">
                                全场:<c:if test="${basketball.mainTeamScore != null}">${basketball.mainTeamScore}:${basketball.guestTeamScore}</c:if>
                            </td>
                            <td align="left">
                                <%--比分:<c:if test="${basketball.mainTeamHalfScore != null}">${basketball.mainTeamHalfScore}:${basketball.guestTeamHalfScore}</c:if>/--%>
                                <%--<c:if test="${basketball.mainTeamScore != null}">${basketball.mainTeamScore}:${basketball.guestTeamScore}</c:if><br/>--%>
                                <%--比分:<c:if test="${basketball.mainTeamScore != null}">${basketball.mainTeamScore}:${basketball.guestTeamScore}</c:if><br/>--%>
                                胜负:<c:if test="${basketball.mainTeamScore != null}">${basketball.mainTeamScore > basketball.guestTeamScore ? "胜" : "负"}</c:if> SP:${basketball.winOrNegaSp}
                                <br />
                                让分胜负:<c:if test="${basketball.mainTeamScore != null}">${basketball.mainTeamScore + basketball.letBall > basketball.guestTeamScore ? "胜" : "负"}</c:if> SP:${basketball.letWinOrNegaSp}
                                <br />
                                胜分差:${myf:sfcScore(basketball.mainTeamScore,basketball.guestTeamScore)} SP:${basketball.winNegaDiffSp}
                                <br />
                                大小分:<c:if test="${basketball.mainTeamScore != null}">${basketball.mainTeamScore + basketball.guestTeamScore > myf:parseDouble(basketball.preCast) ? "大" : "小"}</c:if> SP:${basketball.bigOrLittleSp}
                            </td>
                            <td align="center">
                                <c:if test="${(basketball.endOperator == 1 || basketball.endOperator == 2) && basketball.bonusOperator == 0}">
                                    <a href="#" onclick="toEditOpenBasketball('${basketball.id}');" >开奖中(编辑)</a>
                                </c:if>
                                <c:if test="${basketball.endOperator == 1 && basketball.bonusOperator == 1}">已算奖</c:if>
                    ...
                    <tr>
                        <td colspan="11" align="right">
                            <page:pageTag page="${requestScope.page}" pageSize="${requestScope.pageSize}"
                                          ...
                                          gotoURI="${pageContext.request.contextPath}/manages/openAward?action=openBasketball"/>
                        </td>
                    </tr>
                </c:if>
            </table>
