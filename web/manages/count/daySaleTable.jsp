    ...
    <tr>
        <td style="padding: 4px;" colspan="2">
            <form name="form1" id="form1" method="post"
                  action="${pageContext.request.contextPath}/manages/countServlet?action=daySaleTable">
            ...
            <table width="100%" border="1" cellpadding="4" cellspacing="0" class="content">
                ...
                <c:if test="${pageBean.pageContent==null}">
                    <tr><td colspan="9" align="center">请在上面填写筛选条件，并点击查询按钮来查询数据！</td></tr>
                </c:if>
                <c:if test="${pageBean.pageContent!=null}">
                    <c:forEach var="sub" items="${pageBean.pageContent}" varStatus="cont">
                        <tr align="right">
                            <td align="center">${(page-1)*pageSize+cont.count}</td>
                            <td align="center"><a href="${pageContext.request.contextPath}/manages/countServlet?action=daySaleTableForSub&startDate=${sub.curDate}&endDate=${sub.curDate}" target="_blank">${sub.curDate}</a></td>
                    ...
                    <tr align="right">
                        ...
                        <td>${countMap.succNum}</td>
                        <td><fmt:formatNumber value="${countMap.succAmount}" pattern="0.00"/></td>
                        <td>${countMap.failNum}</td>
                        <td><fmt:formatNumber value="${countMap.failAmount}" pattern="0.00"/></td>
                        <td>${countMap.bonusNum}</td>
                        <td><fmt:formatNumber value="${countMap.fixBonusAmount}" pattern="0.00"/></td>
                        <td><fmt:formatNumber value="${countMap.bonusAmount}" pattern="0.00"/></td>
                    </tr>
                    <tr>
