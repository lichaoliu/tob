            ...
            <form name="form1" method="post"
                ...
                <table width="100%" border="1" cellpadding="4" cellspacing="0" class="seach">
                    ...
                    <tr>
                        ...
                        <td style="width:10%">
                            <select name="bonusOperator">
                                <option value="" <c:if test="${'' == bonusOperator }">selected</c:if>>全部</option>
                                <c:forEach var="award" items="${myf:getBonusOperatorsMap()}">
                                    <option value="${award.key}" <c:if test="${award.key == bonusOperator}">selected</c:if> >${award.value}</option>
                                </c:forEach>
                            </select>
                        </td>
                ...
                <c:if test="${pageBean.pageContent!=null}">
                    <c:forEach var="beiDan" items="${pageBean.pageContent}" varStatus="cont">
                        <tr>
                            ...
                            <td align="center">
                                <c:if test="${beiDan.operatorsAward == 2 && beiDan.bonusOperator == 0}">
                                    <a href="${pageContext.request.contextPath}/manages/calculateAwardServlet?action=beiDanDetail&id=${beiDan.id}">派奖</a>
                                </c:if>
                                <c:if test="${beiDan.operatorsAward == 2 && beiDan.bonusOperator == 1}">
                                    已派奖
                                </c:if>
                            </td>
                        </tr>
