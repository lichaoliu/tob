...
<head>
    ...
    <script type="text/javascript">
        ...
        function toSendWinFootball(id){
            var matchNo= $("#matchNo").val();
            var mainTeam= $("#mainTeam").val();
            var guestTeam= $("#guestTeam").val();
            var matchName= $("#matchName").val();
            var bonusOperator= $("#bonusOperator").val();
            var page= $("#pageId").val();
            window.location.href='${pageContext.request.contextPath}/manages/sendWinManagesServlet?action=footballDetail&id='+id+'&matchNo='+encodeURIComponent(encodeURIComponent(matchNo))+"&mainTeam="+encodeURIComponent(encodeURIComponent(mainTeam))+"&guestTeam="+encodeURIComponent(encodeURIComponent(guestTeam))+"&matchName="+encodeURIComponent(encodeURIComponent(matchName))+"&bonusOperator="+encodeURIComponent(encodeURIComponent(bonusOperator))+"&page="+page;
        }
    </script>
</head>
<body>
    ...
    <tr>
        <td style="padding: 4px;">
            ...
            <form name="form1" method="post"
                  action="${pageContext.request.contextPath}/manages/sendWinManagesServlet?action=footballMatchList">
                <table width="100%" border="1" cellpadding="4" cellspacing="0" class="seach">
                    ...
                    <tr>
                        <td style="width:5%">派奖状态</td>
                        <td style="width:10%">
                            <select name="bonusOperator" id="bonusOperator">
                                <option value="" <c:if test="${'' == bonusOperator }">selected</c:if>>全部</option>
                                <c:forEach var="award" items="${myf:getBonusOperatorsMap()}">
                                    <option value="${award.key}"
                                            <c:if test="${award.key == bonusOperator}">selected</c:if> >${award.value}</option>
                                </c:forEach>
                            </select>
                        </td>
                    ...
                    <c:forEach var="football" items="${pageBean.pageContent}" varStatus="cont">
                        <tr>
                            ...
                            <td align="center"><c:if
                                    test="${football.subIssueForJingCaiZuQiu.letBall != null}">让球${football.subIssueForJingCaiZuQiu.letBall}</c:if></td>
                            <td>
                                让球胜平负:${football.rqSpfResult}
                                <c:if test="${football.subIssueForJingCaiZuQiu.winOrNegaSp != null}">
                                    ,奖金<fmt:formatNumber value="${football.subIssueForJingCaiZuQiu.winOrNegaSp}"
                                                         pattern="0.00"/>元
                                </c:if>
                                <br/>
                                胜平负:${football.spfResult}
                                <c:if test="${football.subIssueForJingCaiZuQiu.spfWinOrNegaSp != null}">
                                    ,奖金<fmt:formatNumber value="${football.subIssueForJingCaiZuQiu.spfWinOrNegaSp}"
                                                         pattern="0.00"/>元
                                </c:if>
                                <br/>
                                ...
                                <c:if test="${football.subIssueForJingCaiZuQiu.totalGoalSp != null}">
                                    ,奖金<fmt:formatNumber value="${football.subIssueForJingCaiZuQiu.totalGoalSp}"
                                                         pattern="0.00"/>元
                                </c:if>
                                <br/>
                                半全场胜平负:${football.bqcspfResult}
                                <c:if test="${football.subIssueForJingCaiZuQiu.halfCourtSp != null}">
                                    ,奖金<fmt:formatNumber value="${football.subIssueForJingCaiZuQiu.halfCourtSp}"
                                                         pattern="0.00"/>元
                                </c:if>
                            </td>
                            <td align="center">
                                <c:if test="${football.subIssueForJingCaiZuQiu.operatorsAward == 2 && football.subIssueForJingCaiZuQiu.bonusOperator == 0}">
                                    <!-- <a href="${pageContext.request.contextPath}/manages/sendWinManagesServlet?action=footballDetail&id=${football.subIssueForJingCaiZuQiu.id}&bonusOperator=${bonusOperator}&startTime=${startTime}&endTime=${endTime}">
                                        派奖</a>-->
                                    <a href="#" onclick="toSendWinFootball('${football.subIssueForJingCaiZuQiu.id}');" >派奖</a>
                                    <br/>
                                    <a href="${pageContext.request.contextPath}/manages/sendWinManagesServlet?action=issueBonusList&lotteryCode=200&sn=${fn:replace(football.subIssueForJingCaiZuQiu.issue,'-' ,'' )}${football.subIssueForJingCaiZuQiu.sn}"
                                       target="_black">中奖票</a>
                                </c:if>
                                <c:if test="${football.subIssueForJingCaiZuQiu.operatorsAward == 2 && football.subIssueForJingCaiZuQiu.bonusOperator == 1}">
                                    已派奖
                                </c:if>
                            </td>
                        </tr>
