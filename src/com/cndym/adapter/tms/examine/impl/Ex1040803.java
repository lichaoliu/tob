 ...
 */
@Component
public class Ex1040803 extends BashExamina implements IExamine {
    private Logger logger = Logger.getLogger(Ex1040803.class);

    @Override
    public void examina(String number, int item) {
//        throw new IllegalArgumentException("该玩法暂不支持");
        String[] nums = number.split(ztag);
        NumberTicketCountExamine.defaulstNumberSortExamine("1040803", nums);
        int temp = 0;
        for (String num : nums) {
            //校验格式
            ...
            NumberLengthExamine.defaultNumberLengthExamine(arr[0], ",", 8, -1);//号码个数的验证
