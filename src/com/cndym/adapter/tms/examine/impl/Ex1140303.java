 ...
 */
@Component
public class Ex1140303 extends BashExamina implements IExamine {
    private Logger logger = Logger.getLogger(Ex1140303.class);

    @Override
    public void examina(String number, int item) {
        String[] nums = number.split(ztag);
        int temp = 0;
        NumberTicketCountExamine.defaulstNumberSortExamine("1140303", nums);
        for (String num : nums) {
	        //校验格式
	        NumberTagExamine.defaultNumberTagExamine(num, new String[]{",", "@"});//号码中分隔符的验证
	        String[] arr = num.split("@");
	        NumberLengthExamine.defaultNumberLengthExamine(arr[0], ",", 3, -1L);//号码个数的验证，胆码应该小于3
    ...
    public static void main(String[] args) {
        Ex1140303 ex1140303 = new Ex1140303();
        ex1140303.examina("01,10@09,08,04,05", 4);
